...
export function QuestionManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingId ? `/api/exam-questions/${editingId}` : "/api/exam-questions"
      ...
      if (data.success) {
        ...
        resetForm()
        setError("")
        alert(editingId ? "Ibibazo byahinduwe neza!" : "Ibibizo byongewe neza!")
      } else {
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <h3 className="text-xl font-semibold text-gray-900 mb-4">
          {editingId ? "Hindura Ibibazo" : "Ohereza Ibibazo Bishya"}
        </h3>
        
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Label htmlFor="question_text">Ikibazo</Label>
            <Textarea
              id="question_text"
      ...
      <Card className="p-6">
        <h3 className="text-xl font-semibold text-gray-900 mb-4">
          Ibibazo Byose ({questions.length})
        </h3>
        
        {questions.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            <p>Nta bibazo byabashyizweho</p>
          </div>
        ) : (
